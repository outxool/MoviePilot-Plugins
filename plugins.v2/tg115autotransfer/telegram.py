...
class TelegramPublicClient:
    ...
    def search(self, channel: str, keyword: str) -> list[TelegramResource]:
        ...
        return self.parse_channel_html(channel=channel, keyword=keyword, html=response.text)
    ...
    def parse_channel_html(self, channel: str, keyword: str, html: str) -> list[TelegramResource]:
        ...
        for wrap in soup.select(".tgme_widget_message_wrap"):
            ...
            sorted_links = sorted(links, key=lambda item: item.key)
            ...
            digest_source = f"{channel}|{message_id}|{text}|{'|'.join(item.key for item in sorted_links)}"
