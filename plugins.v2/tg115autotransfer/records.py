...
class Tg115StateStore:
    ...
    def init_db(self) -> None:
        with self.connect() as conn:
            ...
            self._ensure_transfer_record_columns(conn)

    def _ensure_transfer_record_columns(self, conn: sqlite3.Connection) -> None:
        existing = {row["name"] for row in conn.execute("PRAGMA table_info(transfer_records)").fetchall()}
        columns = {
            "quality_score": "INTEGER DEFAULT 0",
            "resolution": "TEXT DEFAULT ''",
            "quality_flags": "TEXT DEFAULT ''",
        ...
        for name, ddl in columns.items():
            if name not in existing:
                conn.execute(f"ALTER TABLE transfer_records ADD COLUMN {name} {ddl}")
