...
from .follow_schedule import calculate_next_run, parse_follow_schedule_text, search_web_for_follow_schedule
from .matcher import SubscriptionMatcher
from .models import SearchRunContext, SubscriptionInfo, TransferLimits
...
class Tg115AutoTransfer(_PluginBase):
    ...
    _bridge_timer: Optional[Timer] = None
    _state_store: Optional[Tg115StateStore] = None
    ...
    def init_plugin(self, config: Optional[dict] = None):
        ...
        self._max_subscriptions_per_run = max(0, int(config.get("max_subscriptions_per_run") or 0))
        ...
        self._state_store = Tg115StateStore(Path("/config/plugins/tg115autotransfer/state.db"))
    ...
    def on_subscribe_added(self, event: Event = None):
        if not self._enabled or not self._auto_search_on_subscribe or not event or not event.event_data:
            return
        ...
        if old_timer:
            old_timer.cancel()
        timer = Timer(float(self._subscribe_search_delay_seconds), self._run_subscribe_timer, kwargs={"subscribe_id": subscribe_id})
        timer.daemon = True
    ...
    def _run_subscribe_timer(self, subscribe_id: int) -> None:
        try:
            if not self._enabled or not self._auto_search_on_subscribe:
                return
            self.search_subscription_by_id(int(subscribe_id), source="新增订阅自动直搜")
        finally:
            self._subscribe_timers.pop(int(subscribe_id), None)

    def _cancel_all_timers(self) -> None:
        for timer in list(getattr(self, "_subscribe_timers", {}).values()):
            try:
                timer.cancel()
            except Exception:
                pass
        self._subscribe_timers = {}
        self._cancel_bridge_timer()

    def _cancel_bridge_timer(self) -> None:
    ...
    def _searcher(self) -> TgDirectSearcher:
        return TgDirectSearcher(channels=self._channels(), timeout=self._request_timeout, proxy=self._proxy, request_interval_seconds=self._tg_search_interval_seconds, max_keywords_per_subscription=self._max_keywords_per_subscription, retry_limit=self._search_retry_limit)
    ...
    def _format_result(result: Dict[str, Any]) -> str:
        return "\n".join([
            f"订阅总数：{result.get('subscriptions_total', result.get('subscriptions', 0))}",
            f"已处理订阅：{result.get('subscriptions_processed', result.get('subscriptions', 0))}",
            f"剩余订阅：{result.get('subscriptions_remaining', 0)}",
            f"提前停止：{'是' if result.get('stopped_early') else '否'}",
            f"停止原因：{result.get('stop_reason') or '-'}",
            f"TG请求：{result.get('requests', 0)}",
            f"唯一TG消息：{result.get('unique_messages_found', result.get('messages_found', 0))}",
            f"唯一115链接：{result.get('unique_links_found', result.get('links_found', 0))}",
            f"匹配：{result.get('matched', 0)}",
            f"演练：{result.get('previewed', 0)}",
            f"转存：{result.get('transferred', 0)}",
            f"永久跳过：{result.get('skipped_permanent', 0)}",
            f"临时延后：{result.get('deferred', 0)}",
    ...
    def _merge_result(self, target: Dict[str, Any], part: Dict[str, Any]) -> None:
        for key, value in part.items():
            if isinstance(value, bool):
                target[key] = bool(target.get(key)) or value
            elif isinstance(value, int):
                target[key] = int(target.get(key, 0) or 0) + value
            elif key in {"stop_reason"} and value:
                target[key] = value

    def _limits(self, follow_mode: bool = False) -> TransferLimits:
        if follow_mode:
            return TransferLimits(
                max_transfers_per_run=int(self._follow_max_transfers_per_run or 1),
                max_transfers_per_subscription=int(self._follow_max_transfers_per_run or 1),
                max_quality_probes_per_subscription=int(self._max_quality_probe_per_subscription or 0),
            )
        return TransferLimits(
            max_transfers_per_run=int(self._max_transfers_per_run or 0),
            max_transfers_per_subscription=int(self._max_transfers_per_subscription or 0),
            max_quality_probes_per_subscription=int(self._max_quality_probe_per_subscription or 0),
        )

    def _run_search_task(self, subscriptions: list[SubscriptionInfo], source: str, limits: Optional[TransferLimits] = None, follow_mode: bool = False) -> Dict[str, Any]:
        result = self._empty_result()
        subscriptions = list(subscriptions or [])
        result["subscriptions_total"] = len(subscriptions)
        result["subscriptions"] = len(subscriptions)
        if not self._enabled:
            result["stopped_early"] = True
            result["stop_reason"] = "插件未启用"
            return result
        if not self._run_lock.acquire(blocking=False):
            result["stopped_early"] = True
            result["stop_reason"] = "已有任务正在运行，本次未执行"
            result["skipped"] = 1
            logger.warning("〖TG115自动转存〗已有搜索任务正在运行，本次跳过")
            return result
        store = self._store()
        run_id = store.start_run(source=source, channels_count=len(self._channels()), subscriptions_total=len(subscriptions))
        controller = TransferController(self, store, limits or self._limits(follow_mode=follow_mode))
        context = SearchRunContext(source=source, store=store, controller=controller, subscriptions_total=len(subscriptions), total_result=result)
        try:
            for index, sub in enumerate(subscriptions, start=1):
                if not self._enabled:
                    context.request_stop("插件已关闭")
                    break
                if context.stop_requested:
                    break
                part = self._search_subscription(sub, context)
                self._merge_result(result, part)
                result["subscriptions_processed"] = index
                context.subscriptions_processed = index
                if part.get("stopped_early"):
                    context.request_stop(str(part.get("stop_reason") or "任务提前停止"))
                    break
            processed = int(result.get("subscriptions_processed") or 0)
            result["subscriptions_remaining"] = max(0, len(subscriptions) - processed)
            if context.stop_requested:
                result["stopped_early"] = True
                result["stop_reason"] = context.stop_reason
            store.finish_run(
                run_id,
                subscriptions_total=result["subscriptions_total"],
                subscriptions_processed=result["subscriptions_processed"],
                subscriptions_remaining=result["subscriptions_remaining"],
                stopped_early=1 if result["stopped_early"] else 0,
                stop_reason=str(result.get("stop_reason") or ""),
                messages_found=result["messages_found"],
                links_found=result["links_found"],
                raw_messages_found=result["raw_messages_found"],
                raw_links_found=result["raw_links_found"],
                unique_messages_found=result["unique_messages_found"],
            ...
            if context.bridge_required or result.get("transferred", 0) > 0:
                self._schedule_bridge_event()
            self._notify_run_result(result, source=source, stop_reason=str(result.get("stop_reason") or ""))
            return result
        finally:
            controller.close()
            self._run_lock.release()

    def _search_subscription(self, subscription: SubscriptionInfo, context: SearchRunContext) -> Dict[str, Any]:
        result = self._empty_result()
        ...
        for key in ("requests", "search_operations", "retries", "raw_messages_found", "raw_links_found", "messages_found", "links_found", "unique_messages_found", "unique_links_found"):
            result[key] += int(search_stats.get(key, 0) or 0)
        result["failed"] += int(search_stats.get("errors", 0) or 0)
        ...
        for resource in resources:
            msg_key = (resource.channel, int(resource.message_id))
            if msg_key in context.seen_message_keys:
                continue
            context.seen_message_keys.add(msg_key)
            match = matcher.match(resource, subscription)
            if not match.subscription:
                result["skipped"] += len(resource.links)
                continue
            outcome = context.controller.process_resource(resource, subscription, match.score, context=context)
            self._merge_result(result, outcome.result)
            if outcome.stop_subscription:
                logger.info("〖TG115自动转存〗订阅 %s 提前结束：%s", subscription.name, outcome.stop_reason)
                break
            if outcome.stop_run:
                result["stopped_early"] = True
                result["stop_reason"] = outcome.stop_reason
                context.request_stop(outcome.stop_reason)
                break
        return result

    def search_subscription(self, subscription: SubscriptionInfo, source: str = "手动直搜") -> Dict[str, Any]:
        return self._run_search_task([subscription], source=source)

    def search_subscription_by_id(self, subscribe_id: int, source: str = "指定订阅直搜") -> Dict[str, Any]:
        sub = self._subscription_by_id(int(subscribe_id))
        if not sub:
            logger.warning("〖TG115自动转存〗找不到订阅ID=%s", subscribe_id)
            result = self._empty_result()
            result["failed"] = 1
            result["stop_reason"] = "找不到订阅"
            return result
        return self._run_search_task([sub], source=source)

    def search_all_now(self, source: str = "手动全部直搜") -> Dict[str, Any]:
        return self._run_search_task(list(self._subscriptions()), source=source)

    def search_recent_subscribe(self) -> Dict[str, Any]:
        sid = int(self._load_state().get("recent_subscribe_id") or 0)
        if sid <= 0:
            result = self._empty_result()
            result["failed"] = 1
            result["stop_reason"] = "没有最近新增订阅"
            return result
    ...
    def follow_scan_due(self) -> Dict[str, Any]:
        result = self._empty_result()
        if not self._follow_enabled or not self._enabled:
            return result
        store = self._store()
        today = datetime.now().strftime("%Y-%m-%d")
        due_items = store.due_follow_schedules(limit=200)
        for item in due_items:
            schedule = store.reset_follow_daily_counter_if_needed(item, today)
            sid = int(schedule.get("subscribe_id") or 0)
            if int(schedule.get("trigger_count") or 0) >= int(self._follow_daily_trigger_limit or 1):
                store.update_follow_schedule(sid, last_result="达到每日触发上限，等待下一天")
                continue
            sub = self._subscription_by_id(sid)
            if not sub:
                retry_at = (datetime.now() + timedelta(minutes=10)).strftime("%Y-%m-%d %H:%M:%S")
                store.update_follow_schedule(sid, retry_after=retry_at, last_result="找不到订阅，10分钟后重试")
                continue
            if self._run_lock.locked():
                retry_at = (datetime.now() + timedelta(minutes=10)).strftime("%Y-%m-%d %H:%M:%S")
                store.update_follow_schedule(sid, retry_after=retry_at, last_result="运行锁忙，10分钟后重试")
                continue
            store.increment_follow_trigger_count(sid, today)
            part = self._run_search_task([sub], source=f"追更直搜：{schedule.get('title')}", limits=self._limits(follow_mode=True), follow_mode=True)
            self._merge_result(result, part)
            if part.get("stopped_early") and "限流" in str(part.get("stop_reason") or ""):
                controller = TransferController(self, store)
                cooldown = controller.cooldown_until()
                retry_at = cooldown.strftime("%Y-%m-%d %H:%M:%S") if cooldown else (datetime.now() + timedelta(minutes=self._p115_cooldown_minutes)).strftime("%Y-%m-%d %H:%M:%S")
                store.update_follow_schedule(sid, retry_after=retry_at, last_result="115限流，等待冷却")
                break
            next_run = calculate_next_run(str(schedule.get("parsed_days") or "daily"), str(schedule.get("parsed_time") or ""), int(schedule.get("delay_minutes") or self._follow_delay_minutes))
            store.update_follow_schedule(sid, next_run_at=next_run, last_run_at=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), retry_after="", last_result=self._format_result(part))
        return result

    def _schedule_bridge_event(self) -> None:
        if not self._enabled or not self._bridge_enabled:
            return
        self._cancel_bridge_timer()
        timer = Timer(float(self._bridge_delay_seconds), self._send_bridge_event_if_enabled)
        timer.daemon = True
        self._bridge_timer = timer
        timer.start()
        logger.info("〖TG115自动转存〗已安排一次整理桥接事件，延迟 %s 秒", self._bridge_delay_seconds)

    def _send_bridge_event_if_enabled(self) -> None:
        try:
            if self._enabled and self._bridge_enabled:
                eventmanager.send_event(EventType.PluginAction, {"action": self.EVENT_ACTION, "source": "Tg115AutoTransfer"})
                logger.info("〖TG115自动转存〗已发送整理桥接事件")
        finally:
            self._bridge_timer = None

    def _notify_run_result(self, result: dict, source: str, stop_reason: str = "") -> None:
        if not self._notify_enabled:
            return
        should_send = bool(self._notify_scan_summary)
        if not self._notify_empty_scan and not any(int(result.get(key, 0) or 0) > 0 for key in ("transferred", "failed", "matched", "previewed")) and not stop_reason:
            should_send = False
        if any(int(result.get(key, 0) or 0) > 0 for key in ("transferred", "failed")) or stop_reason:
            should_send = True
        if not should_send:
            return
        try:
            title = f"TG115自动转存：{source}"
            text = self._format_result(result)
            mtype = NotificationType.Plugin
            self.post_message(mtype=mtype, title=title, text=text)
        except Exception as err:
            logger.warning("〖TG115自动转存〗发送通知失败，不影响任务结果：%s", err)
    ...
    async def _api_search_all_now(self) -> dict:
        data = await asyncio.to_thread(self.search_all_now, "手动全部直搜")
        return {"code": 1 if data.get("stop_reason") == "已有任务正在运行，本次未执行" else 0, "message": data.get("stop_reason") or "搜索完成", "data": data}

    async def _api_search_recent_subscribe(self) -> dict:
        data = await asyncio.to_thread(self.search_recent_subscribe)
        return {"code": 0, "message": data.get("stop_reason") or "最近新增订阅搜索完成", "data": data}

    async def _api_search_subscription(self, subscribe_id: int) -> dict:
        data = await asyncio.to_thread(self.search_subscription_by_id, int(subscribe_id), "手动指定订阅直搜")
        return {"code": 0, "message": data.get("stop_reason") or "指定订阅搜索完成", "data": data}

    async def _api_follow_scan_due(self) -> dict:
        data = await asyncio.to_thread(self.follow_scan_due)
        return {"code": 0, "message": "追更到点搜索完成", "data": data}

    async def _api_follow_web_lookup(self) -> dict:
        data = await asyncio.to_thread(self.refresh_follow_schedules_from_subscriptions, True)
        return {"code": 0, "message": "更新时间查询完成", "data": data}

    async def _api_clear_records(self) -> dict:
        await asyncio.to_thread(self._store().clear_records)
        return {"code": 0, "message": "处理记录已清理", "data": None}

    async def _api_status(self) -> dict:
        data = await asyncio.to_thread(self._status)
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        return [{"component": "VForm", "content": [
            {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "v0.7.0：完整重构版。演练记录可转真实；全部搜索会遍历所有有效订阅；单轮上限、限流停止、转存间隔在整轮共享；桥接非阻塞且整轮一次；新增状态迁移、预检上限、TG去重/重试、115分页和追更每日限制。"}},
            {"component": "VAlert", "props": {"type": "warning", "variant": "tonal", "text": "建议升级前备份 /config/plugins/tg115autotransfer/state.db。首次使用保持“仅日志演练”开启，确认命中结果后再真实转存。"}},
            {"component": "VDivider", "props": {"class": "my-3"}},
            {"component": "VAlert", "props": {"type": "info", "variant": "tonal", "text": "一、基础设置"}},
            {"component": "VRow", "content": [
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "enabled", "label": "启用插件", "hint": "关闭后不会运行定时任务、新增订阅自动搜索、延迟Timer和桥接事件。", "persistent-hint": True}}]},
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "dry_run", "label": "仅日志演练", "hint": "默认开启，只记录会处理什么，不真实转存；关闭演练后 previewed 记录允许进入真实转存。", "persistent-hint": True}}]},
            ...
            {"component": "VDivider", "props": {"class": "my-3"}},
            {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "二、TG直接搜索设置。v0.7.0 起“搜索全部订阅”不再受旧的每轮订阅数限制；旧字段仅兼容保存，不再截取前20个。"}},
            {"component": "VRow", "content": [
                {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "auto_search_on_subscribe", "label": "新增订阅后自动搜索", "hint": "新增MP订阅后等待指定秒数搜索；执行前会再次检查插件是否仍启用。", "persistent-hint": True}}]},
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "scheduled_search_enabled", "label": "定时搜索全部订阅", "hint": "真正遍历全部有效订阅；达到全局转存上限或限流时会提前停止并显示原因。", "persistent-hint": True}}]},
            ]},
            {"component": "VRow", "content": [
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "search_retry_limit", "label": "TG临时错误重试次数", "type": "number", "persistent-hint": True}}]},
            ]},
            {"component": "VDivider", "props": {"class": "my-3"}},
            {"component": "VAlert", "props": {"type": "warning", "variant": "tonal", "text": "三、去重、匹配和115保护：单轮上限作用于整轮任务；限流后可停止整轮；同一TG消息和同一订阅内同一115分享本轮只处理一次。"}},
            {"component": "VRow", "content": [
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "max_transfers_per_run", "label": "单轮最多转存", "type": "number", "hint": "整个任务共享，不是每个订阅各算一次。", "persistent-hint": True}}]},
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "transfer_delay_seconds", "label": "两次转存间隔秒数", "type": "number", "hint": "所有订阅共享。", "persistent-hint": True}}]},
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "stop_on_rate_limit", "label": "限流后立刻停止整轮", "persistent-hint": True}}]},
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "bridge_enabled", "label": "通知整理桥接", "hint": "整轮最多安排一次，非阻塞。", "persistent-hint": True}}]},
            ]},
            {"component": "VRow", "content": [
            ...
            {"component": "VAlert", "props": {"type": "info", "variant": "tonal", "text": "四、资源质量检测：真实转存前按配置限制每订阅预检数量；达到上限会临时延后，下轮可重试。"}},
            {"component": "VRow", "content": [
            ...
            {"component": "VDivider", "props": {"class": "my-3"}},
            {"component": "VAlert", "props": {"type": "info", "variant": "tonal", "text": "五、电视剧追更：支持单剧每次最多转存和每部剧每日触发次数限制；锁忙不会直接错过，会短期重试。"}},
            {"component": "VRow", "content": [
    ...
    def get_page(self) -> Optional[List[dict]]:
        ...
        stats_text = f"处理记录：{stats.get('records', 0)} ｜ 今日搜索次数：{stats.get('today_runs', 0)} ｜ 已转存：{by_status.get('transferred', 0)} ｜ 演练：{by_status.get('previewed', 0)} ｜ 永久跳过：{by_status.get('skipped_permanent', 0)} ｜ 临时延后：{by_status.get('deferred', 0)} ｜ 重复跳过：{by_status.get('skipped_duplicate', 0)} ｜ 失败：{by_status.get('failed_retryable', 0) + by_status.get('failed_final', 0)} ｜ 追更剧：{stats.get('follow_count', 0)} ｜ 下次追更：{next_follow.get('title', '-') if next_follow else '-'} {next_follow.get('next_run_at', '') if next_follow else ''}"
        return [
            {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "v0.7.0：完整重构版。全部搜索遍历所有有效订阅；演练记录可转真实；全局上限、限流停止、转存间隔、预检上限、状态迁移、非阻塞桥接均已接入。"}},
            {"component": "VAlert", "props": {"type": "warning", "variant": "tonal", "text": "操作区每张卡片都写明用途。真实转存前建议先保持“仅日志演练”开启。升级前请备份 state.db。"}},
            {"component": "VRow", "props": {"class": "my-2"}, "content": [
                {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VCard", "props": {"variant": "outlined", "class": "h-100"}, "content": [
                    ...
                    {"component": "VCardText", "text": "对当前所有活跃订阅逐个搜索TG频道，不再固定截取前20个。达到单轮转存上限或115限流时会提前停止并显示已处理/剩余数量。演练模式下只记录命中结果，不真实转存。"},
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VCard", "props": {"variant": "outlined", "class": "h-100"}, "content": [
                    ...
                    {"component": "VCardText", "text": "只搜索最近一次新增的MP订阅。新增订阅后插件也会默认等待指定秒数自动搜索；关闭插件或保存配置会取消旧Timer。"},
            ...
            {"component": "VRow", "props": {"class": "my-2"}, "content": [
                {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VCard", "props": {"variant": "outlined", "class": "h-100"}, "content": [
                    ...
                    {"component": "VCardText", "text": "只搜索已经到更新时间的电视剧。受单剧每次最多转存和每日触发次数限制；锁忙会短期重试，不会直接错过。"},
            ...
            {"component": "VCard", "props": {"variant": "outlined", "class": "mb-2"}, "content": [
                ...
                {"component": "VCardText", "text": "清空已处理链接和搜索运行摘要。清空后同一资源可能再次被处理，慎用。不会清理追更计划。"},
