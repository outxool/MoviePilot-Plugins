...
from .follow_schedule import (
    calculate_next_run,
    parse_follow_schedule_text,
    search_web_for_follow_schedule,
    weekday_text,
)
...
class Tg115AutoTransfer(_PluginBase):
    ...
    def init_plugin(self, config: Optional[dict] = None):
        ...
        self._follow_web_cache_hours = max(1, int(config.get("follow_web_cache_hours") or 24))
    ...
    def _config_dict(self) -> Dict[str, Any]:
        return {
            ...
            "follow_web_cache_hours": self._follow_web_cache_hours,
    ...
    def get_service(self) -> List[Dict[str, Any]] | None:
        ...
        services = [{
            ...
            "name": "TG公开频道增量扫描、收集TG旧资源与115受控转存",
            "trigger": trigger,
            "func": self.scan_once,
            "kwargs": {},
        }]
        if self._follow_enabled:
            services.append({
                "id": "Tg115AutoTransfer.follow_due",
                "name": "TG115电视剧追更到点单剧搜索",
                "trigger": CronTrigger.from_crontab("*/5 * * * *"),
                "func": self.run_due_follow_schedules,
                "kwargs": {},
            })
        return services
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            ...
            {"path": "/follow_scan_due", "endpoint": self._api_follow_scan_due, "methods": ["POST"], "auth": "bear", "summary": "执行到点追更任务"},
    ...
    def match_history_for_subscription(self, subscribe_id: int, source: str = "单订阅匹配", allow_transfer: bool = True) -> Dict[str, int]:
        result = self._empty_result()
        if not self._history_enabled:
            logger.warning("〖TG115自动转存〗历史资源库未启用，跳过单订阅匹配")
            return result
        subscription = self._subscription_by_id(int(subscribe_id))
        if not subscription:
            logger.warning("〖TG115自动转存〗找不到订阅ID=%s，跳过单订阅匹配", subscribe_id)
            result["skipped"] += 1
            return result
        if not self._run_lock.acquire(blocking=False):
            logger.warning("〖TG115自动转存〗已有任务正在运行，单订阅匹配跳过：%s", subscription.name)
            result["skipped"] += 1
            return result
        try:
            store = self._store()
            ...
            target_cid: str | None = None
            last_transfer_at = 0.0
            logger.info("〖TG115自动转存〗开始单订阅匹配：%s，候选链接=%s，触发方式=%s", subscription.name, len(links), source)
            for link in links:
                resource = TelegramResource(
                    channel=str(link["channel"]),
                    message_id=int(link["message_id"]),
                ...
                if not match.subscription or match.score < self._history_minimum_score:
                    result["skipped"] += 1
                    continue
                allowed, status, reason = self._media_decision(resource, subscription)
                if not allowed:
                    store.update_link_status(int(link["id"]), status, matched_subscription_id=subscription.sid, matched_subscription_name=subscription.name, matched_score=match.score, matched_at=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), error_message=reason)
                    if status == STATUS_EXISTING:
                        result["existing"] += 1
                    elif status == STATUS_NEED_CONFIRM:
                        result["need_confirm"] += 1
                        ...
                        result["skipped"] += 1
                    logger.info("〖TG115自动转存〗单订阅命中但不转存：%s -> %s，原因=%s", resource.title, subscription.name, reason)
                    continue
                result["matched"] += 1
                store.update_link_status(int(link["id"]), STATUS_MATCHED, matched_subscription_id=subscription.sid, matched_subscription_name=subscription.name, matched_score=match.score, matched_at=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), error_message=reason)
                if not allow_transfer or self._dry_run:
                    result["previewed"] += 1
                    continue
                if self._in_cooldown():
                    logger.warning("〖TG115自动转存〗115冷却中，单订阅真实转存暂停")
                    result["skipped"] += 1
                    break
                if result["transferred"] >= self._follow_max_transfers_per_run:
                    logger.info("〖TG115自动转存〗单订阅达到本轮上限 %s，停止", self._follow_max_transfers_per_run)
                    break
                if self._transfer_delay_seconds > 0 and last_transfer_at > 0:
                    wait = self._transfer_delay_seconds - (time.time() - last_transfer_at)
                    if wait > 0:
                ...
                try:
                    ...
                    result["transferred"] += 1
                    last_transfer_at = time.time()
                    self._notify_transfer_result(success=True, resource=resource, subscription=subscription, share_url=str(link["url"]), message=transfer.message)
                except Exception as err:
                    result["errors"] += 1
                    store.increment_retry(int(link["id"]), str(err))
                    logger.error("〖TG115自动转存〗单订阅转存失败 %s: %s", link["url"], err, exc_info=True)
                    if self._is_rate_limited(err):
                        self._set_cooldown(err)
                        if self._stop_on_rate_limit:
                            break
            self._record_scan_result(result, source)
            self._log_scan_summary(result, source)
            return result
        finally:
            self._run_lock.release()


    def trigger_subscription_search(self, subscribe_id: int, source: str = "单订阅自动搜索", allow_transfer: bool = True) -> Dict[str, int]:
        result = self._empty_result()
        if self._history_enabled:
            store = self._store()
            increment = self._scan_increment(self._channels(), store)
            for key, value in increment.items():
                result[key] = result.get(key, 0) + int(value or 0)
        matched = self.match_history_for_subscription(subscribe_id, source=source, allow_transfer=allow_transfer)
        for key, value in matched.items():
            result[key] = result.get(key, 0) + int(value or 0)
        return result
    ...
    def on_subscribe_added(self, event: Event = None):
        if not event or not event.event_data:
            return
        if not self._auto_search_on_subscribe:
            return
        subscribe_id = int((event.event_data or {}).get("subscribe_id") or 0)
        if subscribe_id <= 0:
            return
        logger.info("〖TG115自动转存〗收到新增订阅事件：订阅ID=%s，%s 秒后自动搜索该订阅", subscribe_id, self._subscribe_search_delay_seconds)
        old_timer = self._subscribe_timers.pop(subscribe_id, None)
        if old_timer:
            old_timer.cancel()
        timer = Timer(
            float(self._subscribe_search_delay_seconds),
            self.trigger_subscription_search,
            kwargs={"subscribe_id": subscribe_id, "source": "新增订阅自动搜索", "allow_transfer": True},
        )
        timer.daemon = True
        self._subscribe_timers[subscribe_id] = timer
        timer.start()

    def _build_follow_schedule_for_subscription(self, subscription: SubscriptionInfo, raw_text: str = "", web_lookup: bool = False) -> Optional[Dict[str, Any]]:
        parsed = parse_follow_schedule_text(raw_text or subscription.name, source="手动填写") if self._follow_manual_parse_enabled else None
        if (not parsed or not parsed.matched) and web_lookup and self._follow_web_lookup_enabled:
            parsed = search_web_for_follow_schedule(subscription.name, timeout=self._request_timeout, proxy=self._proxy)
        if not parsed or not parsed.matched:
            return None
        next_run = calculate_next_run(parsed.parsed_days, parsed.parsed_time, self._follow_delay_minutes)
        return {
            "subscribe_id": subscription.sid,
            "title": subscription.name,
            "tmdbid": "",
            "season": int(subscription.season or 0),
            "enabled": True,
            "raw_text": parsed.raw_text,
            "parsed_days": parsed.parsed_days,
            "parsed_time": parsed.parsed_time,
            "episode_count": parsed.episode_count,
            "delay_minutes": self._follow_delay_minutes,
            "source": parsed.source,
            "confidence": parsed.confidence,
            "next_run_at": next_run,
            "last_web_check_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S") if parsed.source == "全网查询" else "",
        }
    ...
    def refresh_follow_schedules_from_subscriptions(self, web_lookup: bool = False) -> Dict[str, int]:
        ...
        if not self._follow_enabled:
            logger.info("〖TG115自动转存〗追更总开关关闭，跳过更新时间识别")
            return result
        store = self._store()
        web_count = 0
        for sub in self._subscriptions():
            ...
            if not is_tv:
                continue
            result["checked"] += 1
            allow_web = bool(web_lookup and self._follow_web_lookup_enabled and web_count < self._follow_web_max_per_run)
            schedule = self._build_follow_schedule_for_subscription(sub, raw_text=sub.keyword or sub.name, web_lookup=allow_web)
            if allow_web:
                web_count += 1
                result["web_checked"] += 1
            if schedule:
                store.upsert_follow_schedule(schedule)
                result["updated"] += 1
                logger.info("〖TG115自动转存〗追更时间已识别：%s，%s %s，延迟%s分钟后搜索", sub.name, weekday_text(schedule["parsed_days"]), schedule["parsed_time"], schedule["delay_minutes"])
            else:
                result["not_matched"] += 1
        return result

    def run_due_follow_schedules(self) -> Dict[str, int]:
        result = self._empty_result()
        if not self._follow_enabled:
            logger.info("〖TG115自动转存〗追更总开关关闭，到点追更任务跳过")
            return result
        store = self._store()
        due = store.due_follow_schedules(limit=20)
        logger.info("〖TG115自动转存〗检查到点追更任务：%s 个", len(due))
        if due:
            increment = self._scan_increment(self._channels(), store)
            for key, value in increment.items():
                result[key] = result.get(key, 0) + int(value or 0)
        for item in due:
            ...
            source = f"追更定时：{item.get('title')}"
            one = self.match_history_for_subscription(subscribe_id, source=source, allow_transfer=True)
            ...
            store.update_follow_schedule(subscribe_id, next_run_at=next_run, last_run_at=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), last_result=str(one)[:1000])
        return result
    ...
    def _status(self) -> dict:
        ...
        return {
            ...
            "follow_web_lookup_enabled": self._follow_web_lookup_enabled,
            "follow_delay_minutes": self._follow_delay_minutes,
            "follow_schedules": len(self._store().list_follow_schedules()) if self._history_enabled else 0,
    ...
    async def _api_follow_scan_due(self) -> dict:
        logger.info("〖TG115自动转存〗收到手动执行到点追更任务请求")
        return {"code": 0, "message": "到点追更任务执行完成", "data": self.run_due_follow_schedules()}

    async def _api_follow_web_lookup(self) -> dict:
        logger.info("〖TG115自动转存〗收到全网查询电视剧更新时间请求")
        return {"code": 0, "message": "全网查询完成，追更结果已写入追更列表", "data": self.refresh_follow_schedules_from_subscriptions(web_lookup=True)}
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        return [{
            "component": "VForm",
            "content": [
                {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "v0.4.2 更新：按钮改成“图标+文字+说明”；定时改用 MoviePilot 标准可点选控件；新增订阅后默认延迟30秒自动搜索；追更功能有总开关（默认关闭），可选全网自动查询更新时间（默认关闭），更新时间后默认35分钟单剧搜索。"}},
                ...
                {"component": "VAlert", "props": {"type": "warning", "variant": "tonal", "text": "建议第一次使用：先打开“仅日志演练”，让它只写日志不真实转存；确认匹配没问题后，再关闭演练。“收集TG旧资源”默认只入库，不建议一上来就批量真实转存。"}},
                ...
                {"component": "VDivider", "props": {"class": "my-3"}},
                {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "三、TG历史资源库：把TG以前发过的115链接慢慢保存下来。以后你新加老电影/老剧订阅，就能从这些旧资源里找，不用等TG频道重新发。"}},
                {"component": "VRow", "content": [
                    ...
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "auto_backfill_history", "label": "自动收集TG旧资源", "hint": "每次运行顺手往前翻几页TG旧消息，只保存资源链接到本地库。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VSwitch", "props": {"model": "backfill_transfer_enabled", "label": "收集旧资源时就转存", "hint": "不建议开启。收集旧资源可能命中很多资源，容易触发115限流。", "persistent-hint": True}}]},
                ...
                {"component": "VRow", "content": [
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "backfill_pages_per_run", "label": "每轮收集几页TG旧资源", "type": "number", "hint": "越大越快，但越容易被TG限制。建议5。", "persistent-hint": True}}]},
                    ...
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "backfill_page_delay_seconds", "label": "翻页间隔秒数", "type": "number", "hint": "每翻一页TG旧消息等多久。建议2秒以上。", "persistent-hint": True}}]},
                ...
                {"component": "VDivider", "props": {"class": "my-3"}},
                {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "五、新增订阅自动搜索：你在MP新增订阅后，插件默认等30秒，只针对这个订阅搜索TG历史资源和最新资源。"}},
                {"component": "VRow", "content": [
                    {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VSwitch", "props": {"model": "auto_search_on_subscribe", "label": "新增订阅后自动搜索TG资源", "hint": "默认开启。新增MP订阅后自动搜索这个订阅，不用手动点重新匹配。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VTextField", "props": {"model": "subscribe_search_delay_seconds", "label": "新增订阅后等待秒数", "type": "number", "hint": "默认30秒。等MP先保存订阅和缺失集，再开始搜索。", "persistent-hint": True}}]},
                ]},

                {"component": "VDivider", "props": {"class": "my-3"}},
                {"component": "VAlert", "props": {"type": "warning", "variant": "tonal", "text": "六、电视剧追更搜索：总开关默认关闭。开启后，插件可以识别电视剧更新时间，并在更新时间后35分钟只搜索这部剧。全网自动查询也有单独开关，默认关闭。"}},
                {"component": "VRow", "content": [
                    {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "follow_enabled", "label": "启用电视剧追更搜索", "hint": "追更总开关，默认关闭。关闭时所有追更识别和单剧定时搜索都不运行。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "follow_manual_parse_enabled", "label": "识别手动填写的更新时间", "hint": "识别“每日21:30更新”“每晚21:30两集连播”等文本。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 4}, "content": [{"component": "VSwitch", "props": {"model": "follow_web_lookup_enabled", "label": "自动上网查询更新时间", "hint": "默认关闭。开启后会从公开网页搜索更新时间，结果可能不准，可手动修正。", "persistent-hint": True}}]},
                ]},
                {"component": "VRow", "content": [
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "follow_delay_minutes", "label": "更新时间后延迟搜索分钟", "type": "number", "hint": "默认35分钟。默认35分钟。例如21:30更新，就22:05搜索。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "follow_max_transfers_per_run", "label": "单剧每次最多转存", "type": "number", "hint": "默认1条，继续保护115。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "follow_daily_trigger_limit", "label": "每部剧每天最多触发", "type": "number", "hint": "默认2次，防止重复搜索。", "persistent-hint": True}}]},
                    {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VTextField", "props": {"model": "follow_web_max_per_run", "label": "每轮最多联网查几部", "type": "number", "hint": "默认3部，避免高频访问网页。", "persistent-hint": True}}]},
                ]},

                {"component": "VDivider", "props": {"class": "my-3"}},
                {"component": "VAlert", "props": {"type": "info", "variant": "tonal", "text": "七、定时和通知：定时任务使用 MoviePilot 标准时间选择器，不需要手写Cron。通知走MP现有通知渠道。"}},
                {"component": "VRow", "content": [
                    {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VCronField", "props": {"model": "cron", "label": "自动扫描周期", "placeholder": "点击选择，例如每15分钟、每小时、每天固定时间", "hint": "使用MP标准定时选择器，不需要手写 */15 * * * *。", "persistent-hint": True}}]},
    ...
    def get_page(self) -> Optional[List[dict]]:
        ...
        channel_text = "\n".join(channel_lines) or "还没有TG旧资源收集记录。点“收集TG旧资源”后这里会显示每个频道的进度。"
        cooldown_text = "正常"
        ...
        return [
            {"component": "VAlert", "props": {"type": "success", "variant": "tonal", "text": "v0.4.2 更新：按钮增加中文功能名和说明；定时改成MP标准时间选择器；新增订阅默认30秒后自动搜索；追更功能增加总开关，默认关闭；全网查询更新时间也默认关闭；追更延迟默认35分钟。"}},
            {"component": "VAlert", "props": {"type": "info", "variant": "tonal", "text": "怎么用：1）先配置TG频道和115 Cookie；2）先打开“仅日志演练”；3）点“收集TG旧资源”把TG以前发过的115资源存进库；4）新增MP订阅后插件会自动延迟搜索；5）确认日志没问题后再关闭演练真实转存。"}},
            ...
            {"component": "VRow", "props": {"class": "my-2"}, "content": [
                ...
                {"component": "VCol", "props": {"cols": 12, "md": 3}, "content": [{"component": "VBtn", "props": {"color": "info", "variant": "tonal", "block": True, "prepend-icon": "mdi-history", "text": "收集TG旧资源"}, "events": {"click": {"api": "plugin/Tg115AutoTransfer/backfill_now", "method": "post"}}}]},
            ...
            {"component": "VRow", "props": {"class": "my-2"}, "content": [
                {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VBtn", "props": {"color": "warning", "variant": "outlined", "block": True, "prepend-icon": "mdi-rewind", "text": "重置旧资源收集进度"}, "events": {"click": {"api": "plugin/Tg115AutoTransfer/reset_backfill", "method": "post"}}}]},
            ...
            {"component": "VCard", "props": {"variant": "outlined", "class": "mb-2"}, "content": [
                {"component": "VCardTitle", "text": "TG历史资源库里现在有什么"},
                {"component": "VCardText", "text": history_text},
            ]},
            {"component": "VCard", "props": {"variant": "outlined", "class": "mb-2"}, "content": [
                {"component": "VCardTitle", "text": "每个TG频道旧资源收集到哪里了"},
                {"component": "VCardText", "text": channel_text},
            ]},
            {"component": "VCard", "props": {"variant": "outlined", "class": "mb-2"}, "content": [
                {"component": "VCardTitle", "text": "电视剧追更搜索"},
                {"component": "VCardText", "text": f"总开关：{'开启' if self._follow_enabled else '关闭'} ｜ 全网查询：{'开启' if self._follow_web_lookup_enabled else '关闭'} ｜ 更新时间后延迟：{self._follow_delay_minutes}分钟 ｜ 已识别追更剧：{len(self._store().list_follow_schedules()) if self._history_enabled else 0}部"},
            ]},
            {"component": "VRow", "props": {"class": "my-2"}, "content": [
                {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VBtn", "props": {"color": "secondary", "variant": "tonal", "block": True, "prepend-icon": "mdi-calendar-search", "text": "执行到点追更搜索"}, "events": {"click": {"api": "plugin/Tg115AutoTransfer/follow_scan_due", "method": "post"}}}]},
                {"component": "VCol", "props": {"cols": 12, "md": 6}, "content": [{"component": "VBtn", "props": {"color": "secondary", "variant": "outlined", "block": True, "prepend-icon": "mdi-web", "text": "全网查询更新时间"}, "events": {"click": {"api": "plugin/Tg115AutoTransfer/follow_web_lookup", "method": "post"}}}]},
            ]},
