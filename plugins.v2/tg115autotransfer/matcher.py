...
class SubscriptionMatcher:
    ...
    def match(self, resource: TelegramResource, subscription: SubscriptionInfo) -> MatchResult:
        ...
        if self.is_tv(subscription) and sub_season and res_season is not None and sub_season != res_season:
            return MatchResult(subscription=None, score=0, reasons=[f"季不一致:订阅S{sub_season},资源S{res_season}"])
        ...
        if self.is_tv(subscription) and sub_season and res_season is None:
            reasons.append(f"资源未显式标季，按订阅S{sub_season}上下文处理")
