...
class ConfigManager(BaseModel):
    ...
    def _get_default_plugin_config_path() -> Path:
        ...
        返回默认的插件配置目录路径
        """
        return settings.PLUGIN_DATA_PATH / "p115strmsync"
    ...
    def _get_default_plugin_db_path() -> Path:
        ...
        """
        return (
            ConfigManager._get_default_plugin_config_path() / "p115strmsync_file.db"
        )
    ...
    def _get_default_plugin_database_script_location() -> Path:
        ...
        返回默认的插件数据库结构目录路径
        """
        return settings.ROOT_PATH / "app" / "plugins" / "p115strmsync" / "database"
    ...
    PLUSIN_NAME: str = Field(
        default="P115StrmSync", min_length=1, description="插件名称"
    )
