...
from ..db_manager import ct_db_manager, P115StrmSyncBase
...
def init_db(engine):
    ...
    # 全量建表
    P115StrmSyncBase.metadata.create_all(engine)
...
def migration_db(db_path, script_location, version_locations: list):
    ...
    更新数据库
    """
    # 启动时的更新，调整从 /config/plugins/p115strmsync/database/versions 中读取持久化的迁移脚本)
    meta_data_path = Path(
        settings.ROOT_PATH
        / "app"
        / "plugins"
...
def init_migration_scripts() -> bool:
    ...
    target_path = Path(
        settings.PLUGIN_DATA_PATH / "p115strmsync" / "database" / "versions"
    )
