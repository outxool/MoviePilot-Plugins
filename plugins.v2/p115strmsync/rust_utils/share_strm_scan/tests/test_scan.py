...
class TestShareStrmScan(unittest.TestCase):
    ...
    def test_scan_nested_dedup_and_non_share_strm(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            sub.mkdir()
            url = (
                "http://127.0.0.1:3000/api/v1/plugin/P115StrmSync/redirect_url"
                "?share_code=sc1&receive_code=r1&id=99"
            )
            (sub / "a.strm").write_text(url + "\n", encoding="utf-8")
            (sub / "dup.strm").write_text(url + "\n", encoding="utf-8")
            (d / "pick.strm").write_text(
                "http://x/api/v1/plugin/P115StrmSync/redirect_url?pickcode=abc\n",
                encoding="utf-8",
            )
    ...
    def test_multiline_two_pairs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            base = (
                "http://h/api/v1/plugin/P115StrmSync/r"
...
class TestShareStrmScanCache(unittest.TestCase):
    ...
    def test_paths_for_without_prior_scan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            url = (
                "http://h/api/v1/plugin/P115StrmSync/r?share_code=aa&receive_code=bb"
            )
    ...
    def test_paths_for_many_and_missing_pair(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            u1 = "http://h/api/v1/plugin/P115StrmSync/r?share_code=a&receive_code=b"
            u2 = "http://h/api/v1/plugin/P115StrmSync/r?share_code=c&receive_code=d"
    ...
    def test_invalidate_then_paths_still_work(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            url = "http://h/api/v1/plugin/P115StrmSync/r?share_code=z&receive_code=w"
