...
const NEEDLE_P115: &[u8] = b"P115StrmSync";
...
mod tests {
    ...
    fn gate_requires_plugin_name() {
        ...
        assert!(gate_share_strm(
            b"http://h/api/v1/plugin/P115StrmSync/x?share_code=a&receive_code=b"
        ));
    }

    #[test]
    fn extract_query_fallback_like_python() {
        let line_with_plugin = "fooP115StrmSync?share_code=x&receive_code=y";
    ...
    #[test]
    fn full_url_parse() {
        let line = "http://127.0.0.1:3000/api/v1/plugin/P115StrmSync/redirect_url?share_code=sc&receive_code=rc&id=1";
