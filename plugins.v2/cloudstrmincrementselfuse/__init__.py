...
class CloudStrmIncrementSelfUse(_PluginBase):
    ...
    @staticmethod
    def __normalize_storage_path(path: str) -> str:
        """
        归一化 MoviePilot 储存插件内路径：始终使用 / 开头，非根目录去掉末尾 /
        """
        current_path = str(path or "/").replace("\\", "/").strip()
        if not current_path.startswith("/"):
            current_path = f"/{current_path}"
        if len(current_path) > 1 and current_path.endswith("/"):
            current_path = current_path.rstrip("/")
        return current_path or "/"

    def __get_storage_file_item(self, storage_chain, storage_name: str, path: str):
        """
        按 115网盘STRM助手/MP StorageChain 的真实链路获取 FileItem：
        StorageChain().get_file_item(storage=储存名, path=Path(云盘路径))。
        CloudDrive2储存插件会通过 StorageOperSelection 事件注入对应 storage_oper。
        """
        current_path = self.__normalize_storage_path(path)
        try:
            return storage_chain.get_file_item(storage=storage_name, path=Path(current_path))
        except TypeError:
            # 兼容少数版本 path 只接受字符串的情况
            return storage_chain.get_file_item(storage=storage_name, path=current_path)

    def __list_storage_files(self, storage_chain, storage_name: str, path: str):
        """
        通过 MoviePilot StorageChain + CloudDrive2储存插件列出目录。
        不再猜 list(storage,path) 这类非标准接口，避免绕过储存插件注入链路。
        """
        file_item = self.__get_storage_file_item(storage_chain, storage_name, path)
        if not file_item:
            raise FileNotFoundError(f"储存 {storage_name} 中不存在目录：{self.__normalize_storage_path(path)}")
        ...
        if not is_dir:
            raise NotADirectoryError(f"储存 {storage_name} 路径不是目录：{self.__normalize_storage_path(path)}")
        try:
            files = storage_chain.list_files(fileitem=file_item)
        except TypeError:
            files = storage_chain.list_files(file_item)
        return files or []
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        return [
            ...
            {
                "path": "/browse_storage_dir",
                "endpoint": self.browse_storage_dir_api,
                "methods": ["GET"],
                "auth": "bear",
                "summary": "浏览CloudDrive2储存目录",
            }
        ]

    def browse_dir_api(self, path: str = Query(default="/", description="目录路径")) -> Dict[str, Any]:
        """
        浏览 MoviePilot 容器内本地目录，用于前端目录选择
        """
        return self.__browse_dir(path)

    def browse_storage_dir_api(
        self,
        storage: str = Query(default="", description="CloudDrive2储存名称"),
        path: str = Query(default="/", description="云盘目录路径")
    ) -> Dict[str, Any]:
        """
        浏览 MoviePilot StorageChain 中的 CloudDrive2 储存目录
        """
        return self.__browse_storage_dir(storage=storage, path=path)

    def __browse_storage_dir(self, storage: str = "", path: str = "/") -> Dict[str, Any]:
        try:
            storage_name = str(storage or "").strip()
            if not storage_name:
                return {"code": 1, "msg": "未填写CloudDrive2储存名称", "data": {"path": path or "/", "items": []}}
            current_path = self.__normalize_storage_path(path)
            ...
            return {"code": 0, "msg": "success", "data": {"path": current_path, "items": items}}
        except Exception as err:
            logger.error(f"浏览CloudDrive2储存目录失败: storage={storage}, path={path}, error={err}")
            return {"code": 1, "msg": f"浏览CloudDrive2储存目录失败：{err}。请确认 CloudDrive2储存插件已启用，储存名称与插件内显示一致，且云盘路径存在。", "data": {"path": path or "/", "items": []}}
