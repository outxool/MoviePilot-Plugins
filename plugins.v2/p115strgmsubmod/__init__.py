...
class P115StrgmSubMod(_PluginBase):
    """115网盘订阅追更插件"""
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "p115strgmsubmod_"
    plugin_order = 20
    auth_level = 1
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            if self._cron:
                ...
                if not ok:
                    logger.warning(
                        f"Cron 过于频繁（要求间隔>= {self._MIN_INTERVAL_HOURS}h）：{self._cron}，已回退默认 30 */2 * * *"
                    )
                    self._cron = "30 */2 * * *"
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._cron and self._cron_interval_ge_min_hours(self._cron, self._MIN_INTERVAL_HOURS):
            try:
                ...
                logger.warning(f"Cron 表达式无效：{self._cron}，将回退 interval=2h。错误：{e}")
                services.append({
                    "id": "P115StrgmSubMod",
                    "name": "115网盘订阅追更服务",
                    "trigger": "interval",
        ...
        if self._hdhive_checkin_enabled and self._hdhive_checkin_cron:
            try:
                services.append({
                    "id": "P115StrgmSubMod_HDHiveCheckin",
                    "name": "HDHive 签到服务",
    ...
    def _do_sync(self) -> bool:
        # 至少启用一个搜索源
        if not self._pansou_enabled and not self._hdhive_enabled:
            logger.error("搜索源均未启用（PanSou/HDHive），无法执行")
            if self._notify:
                self.post_message(
                    mtype=NotificationType.Plugin,
                    title="【115网盘订阅追更】配置错误",
                    text="PanSou、HDHive 均未启用，请至少启用一个搜索源。"
                )
            return False
