...
class P115StrmSync(_PluginBase):
    # 插件名称
    plugin_name = "115网盘STRM同步"
    # 插件描述
    plugin_desc = "基于原版 P115StrmHelper 修改，由 outxool/moviepilot-plugins 维护，独立保留 full/increment sync 主链"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/jxxghp/MoviePilot-Frontend/refs/heads/v2/src/assets/images/misc/u115.png"
    # 插件版本
    plugin_version = VERSION
    # 插件作者
    plugin_author = "outxool（基于 DDSRem 原版修改）"
    # 作者主页
    author_url = "https://github.com/outxool/moviepilot-plugins"
    # 插件配置项ID前缀
    plugin_config_prefix = "p115strmsync_"
    # 加载顺序
    plugin_order = 99
    # 可使用的用户级别
    auth_level = 1
    ...
    def get_service(self) -> List[Dict[str, str | Dict[Any, Any] | Any]] | None:
        ...
        cron_service = [
            {
                "id": "P115StrmSync_offline_status",
                "name": "监控115网盘离线下载进度",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "P115StrmSync_monitor_life_guard",
                    "name": "115生活事件线程守护",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "P115StrmSync_full_sync_strm_files",
                    "name": "定期全量同步115媒体库",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "P115StrmSync_share_strm_cleanup",
                    "name": "定时分享STRM失效清理扫描",
                    "trigger": CronTrigger.from_crontab(
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "P115StrmSync_increment_sync_strm",
                    "name": "115网盘定期增量同步",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "P115StrmSync_hdhive_checkin",
                    "name": "HDHive 签到调度",
        ...
        if configer.strm_backup_enabled and configer.strm_backup_items:
            for backup_item in configer.strm_backup_items:
                if (
                    ...
                    cron_service.append(
                        {
                            "id": f"P115StrmSync_strm_backup_{backup_item.name}",
...
"""
    ...
    def rename_dict_supplement(self, event: Event) -> None:
        ...
        def share_strm_center(url: str) -> Optional[Dict[str, Any]]:
            for i in ["P115StrmSync", "share_code=", "receive_code=", "id="]:
                if i not in url:
                    return None
            try:
        ...
        if source_item.storage == "local":
            ...
            params["url"] = (
                f"http://127.0.0.1:{settings.PORT}/api/v1/plugin/P115StrmSync/redirect_url/{source_item.fileid}"
            )
